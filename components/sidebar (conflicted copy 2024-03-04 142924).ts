...
export class SidebarView extends ItemView {
	...
	constructor(leaf: WorkspaceLeaf, belongsTo: any) {
		super(leaf);
	...
	async onOpen(): Promise<void> {
		...
		this.belongsTo.sidebarActive = true;
	}
	...
	async setState(state: any): Promise<void> {
		if (!state || !state.updateName) return;
		...
		updateButton.style.color = "var(--text-normal)";
		updateButton.style.padding = "0.5rem";
		updateButton.style.borderRadius = "0.5rem";
		updateButton.style.border = "none";
		updateButton.style.cursor = "pointer";
		updateButton.style.fontSize = "1rem";
		updateButton.style.marginBottom = "0.5rem";
		updateButton.addEventListener("click", this.belongsTo.updateBacklinks.bind(this.belongsTo));
		...
		for (const [fileIndex, backlink] of Object.entries(backlinks)) {
